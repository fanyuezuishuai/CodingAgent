...
特色：未使用 Agent 框架。自行实现 tool calling 解析、多轮消息历史与确定性上下文压缩、六个本地工具、参数校验、有界循环、重复调用/最大步数终止、命令审批与超时、工作区路径隔离、验证状态及脱敏 JSONL 轨迹。Web GUI 支持多轮会话、安全 Markdown、文件上传、折叠工具过程、可读审批和停止；同一工作区禁止并发任务。GitHub Actions 在 Windows/Linux 自动测试。
