...
| 事务式回滚 | 修改前保存快照、运行结束封存最终状态；可接受或回滚文件工具改动，并拒绝覆盖运行后的新修改 |
...
运行结束时，CLI/Web 展示的修改文件、统一 Diff、命令退出码、验证状态和终止原因来自本地运行时，而不是模型的文字自述。`verify_command_passed` 只表示模型选择且用户批准的验证命令返回 0；Proof 会保留真实 argv，并明确说明它不等于验证充分。Proof 不包含 `reasoning_content`。
...
普通会话和项目消息保存在当前 Web 服务进程内存中。普通会话之间隔离；项目内的多个对话共用该项目的消息历史，上传文件也会自动带入后续项目任务。历史过长时，模型请求会被 `ContextManager` 压缩；服务重启后这些内存历史和项目都会清空。
...
- 如果目标文件在运行结束后又被用户或其他程序修改，回滚会在写入任何文件前整体拒绝，避免覆盖较新的内容；
